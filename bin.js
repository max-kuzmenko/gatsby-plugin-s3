...
const guessRegion = (s3, constraint) => {
    var _a;
    return (_a = constraint !== null && constraint !== void 0 ? constraint : s3.config.region) !== null && _a !== void 0
        ? _a
        : aws_sdk_1.config.region;
};
...
exports.deploy = async ({ yes, bucket, userAgent } = {}) => {
    var _a, _b;
    const spinner = ora_1
        .default({ text: 'Retrieving bucket info...', color: 'magenta', stream: process.stdout })
        .start();
    let dontPrompt = yes;
    const uploadQueue = [];
    try {
        ...
        httpOptions = Object.assign(
            {
                agent: process.env.HTTP_PROXY ? proxy_agent_1.default(process.env.HTTP_PROXY) : undefined,
                                ...
                                params: Object.assign(
                                    {
                                        ...
                                        ACL:
                                            config.acl === null
                                                ? undefined
                                                : (_a = config.acl) !== null && _a !== void 0
                                                ? _a
                                                : 'public-read',
                                        ContentType:
                                            (_b = mime_1.default.getType(path)) !== null && _b !== void 0
                                                ? _b
...
{dim   Created Redirect {cyan ${key}} => {cyan ${redirectLocation}}}\n`;
        ...
        if (config.removeNonexistentObjects) {
            const persistObjects = ((_b = config.retainObjectsPatterns) !== null && _b !== void 0 ? _b : []).map(glob =>
                glob_to_regexp_1.default(glob, { globstar: true, extended: true })
            );
        ...
        if (config.enableS3StaticWebsiteHosting) {
            const s3WebsiteDomain = util_2.getS3WebsiteDomainUrl(
                region !== null && region !== void 0 ? region : 'us-east-1'
            );
            console.log(chalk_1.default`
